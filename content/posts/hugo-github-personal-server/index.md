...
- **SSL/TLS 证书**：证书用于加密你与外部客户端之间的通信。当你在外地通过浏览器或 RDP 客户端连接到你的服务时，SSL/TLS 证书会确保所有的数据都是加密传输的，防止被中间人攻击或数据泄露。
    ...
    - **外地连接**：当你从外地的陌生电脑连接到 `villion.zicp.fun:4800` 时，如果你已经配置了正确的证书，连接就会是加密的。如果没有证书，你的浏览器可能会显示警告，提示“连接不安全”之类的信息。你可以选择信任自签名证书或者使用受信任的证书颁发机构（如 Let's Encrypt）签发的证书。
...
![](25.png)
### ✅ 特别注意：**Common Name（CN）一定要写你的域名：否则浏览器/RDP 客户端会报证书名不一致。我图中由于test就会回车了。
...
- **nginx监听端口****：这个端口是你用来通过 Nginx 反向代理 RDP 服务的端口。因为 RDP 协议（远程桌面协议）使用的是 TCP 协议，而你并不希望直接将 RDP 端口（4777）暴露到公网上，Nginx 会帮助你将 RDP 流量从 4800 端口代理到内网的 4777 端口。这样可以有效地隐藏你的实际 RDP 端口，增加安全性。
    

因此，映射 **443** 用于 HTTPS，映射 **4800** 用于反向代理 RDP。这样做的目的是：
